...
def extract_reviews(
    base_path: str = ".",
    split: str = "train",
    total_reviews: int = 1750,
    pos_proportion: float = 0.5,
) -> List[Tuple[str, int]]:
    """
    ...
    assert num_positive <= len(
        pos_reviews
    ), f"unable to extract {num_positive} positive reviews"
    assert num_negative <= len(
        neg_reviews
    ), f"unable to extract {num_negative} negative reviews"
...
if __name__ == "__main__":
    ...
    train_reviews = extract_reviews(
        split="train", pos_proportion=0.5, total_reviews=1650
    )
    ...
    print(
        "positive reviews in train: ", sum(label for (review, label) in train_reviews)
    )
    ...
    for split_name, split_data in zip(
        ["train", "dev", "test"], [train_reviews, dev_reviews, test_reviews]
    ):
